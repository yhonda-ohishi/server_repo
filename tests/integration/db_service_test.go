...
func TestDBServiceIntegration(t *testing.T) {
	...
	t.Run("ETCMeisaiService", func(t *testing.T) {
		...
		assert.Greater(t, createResp.EtcMeisai.Id, int64(0))
	...
	t.Run("DTakoUriageKeihiService", func(t *testing.T) {
		...
		assert.Contains(t, createResp.DtakoUriageKeihi.SrchId, "SRCH")
	})

	t.Run("DTakoFerryRowsService", func(t *testing.T) {
		...
		assert.Greater(t, createResp.DtakoFerryRows.Id, int32(0))
	})

	t.Run("ETCMeisaiMappingService", func(t *testing.T) {
		...
		assert.Greater(t, createResp.EtcMeisaiMapping.Id, int64(0))
	})
}